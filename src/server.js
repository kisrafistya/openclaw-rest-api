...
// Provides /v1/chat/completions that bridges to Gemini API
// ============================================================================
...
function requireApiAuth(req, res, next) {
  ...
  if (scheme !== "Bearer" || !token) {
    return res.status(401).json({ error: { message: "Missing Bearer token", type: "auth_error" } });
  }
  
  if (token !== OPENCLAW_GATEWAY_TOKEN) {
    return res.status(401).json({ error: { message: "Invalid token", type: "auth_error" } });
  }
...
function geminiToOpenai(geminiResponse, model) {
  ...
  const finishReason = geminiResponse.candidates?.[0]?.finishReason?.toLowerCase() || "stop";
...
app.post("/v1/chat/completions", requireApiAuth, async (req, res) => {
  try {
    if (!GOOGLE_API_KEY) {
      return res.status(500).json({ 
        error: { message: "GOOGLE_API_KEY not configured", type: "server_error" } 
      });
    }
    ...
    const geminiModel = modelMapping[model] || "gemini-2.0-flash";
    
    // Convert to Gemini format
    const { contents, systemInstruction } = openaiToGemini(messages);
    ...
    if (max_tokens) {
      geminiPayload.generationConfig.maxOutputTokens = max_tokens;
    }
    ...
    if (!geminiRes.ok) {
      console.error("[/v1/chat/completions] Gemini error:", geminiData);
    ...
    const openaiResponse = geminiToOpenai(geminiData, model);
...
app.get("/v1/models", requireApiAuth, (_req, res) => {
  res.json({
    object: "list",
    data: [
      { id: "gemini-2.0-flash", object: "model", owned_by: "google" },
      { id: "gpt-4", object: "model", owned_by: "openai-compat" },
      { id: "gpt-3.5-turbo", object: "model", owned_by: "openai-compat" }
    ]
  });
});
